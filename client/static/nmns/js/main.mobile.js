...
(function($) {
    ...
    NMNS.calendar = new tui.Calendar("#mainCalendar", {
        ...
        theme: {
            ...
            "week.timegridOneHour.height": "136px",
            "week.timegridHalfHour.height": "68px",
    ...
    NMNS.socket.on("get manager", socketResponse("담당자 정보 받아오기", function(e) {
        ...
        $("#mainTaskContents .task").off('touch click').on('touch click', function(e){
          ...
          switchMenu.call($(".taskDetailLink"), e);
    ...
    function initNoShowModal() {
        if (!$("#noShowScheduleList").hasClass("ps")) {
          ...
          $("#noShowAddBtn").off("touch click").on("touch click", function() {
            ...
            NMNS.emit("add noshow", { id: NMNS.email + generateRandom(), contact: $("#noShowAddContact").val(), noShowCase: $("#noShowAddContent .bg-primary").length === 0 ? $("#noShowAddCaseEtc").val().trim() : $("#noShowAddContent .bg-primary").data("value") });
          });
    ...
    */
    ...
    function initTaskTab(task) {
      if (!$(".taskDetailMenu").data('inited')) {
        $(".taskDetailMenu").data("inited", true);
        ...
        $("#taskStartDate").on("touch click", function(){
          $("#endTaskArea").hide();
          $("#startTaskArea").show();
          this._flatpickr.setDate(moment($(this).val(), 'YYYY. MM. DD').toDate());
        });
        $("#taskEndDate").on("touch click", function(){
          $("#startTaskArea").hide();
          $("#endTaskArea").show();
          this._flatpickr.setDate(moment($(this).val(), 'YYYY. MM. DD').toDate());
        });
        flatpickr("#taskStartDate", {
          dateFormat: "Y. m. d",
          disableMobile: true,
          appendTo:document.getElementById('startTaskArea'),
          locale: "ko",
          onChange:function(selectedDates, dateStr, instance){
            $("#taskCalendarModal").modal('hide');
          }
        });
        flatpickr("#taskEndDate", {
          dateFormat: "Y. m. d",
          disableMobile: true,
          appendTo:document.getElementById('endTaskArea'),
          locale: "ko",
          onChange:function(selectedDates, dateStr, instance){
            $("#taskCalendarModal").modal('hide');
          }
        ...
        $("#taskBtn").on("touch click", function() {
          ...
          var origin = $(".taskDetailMenu").data("task");
          origin.manager = origin.calendarId;
          NMNS.history.push(origin);
...
/*
    ...
    $('.announcementMenuLink').on('touch click', function(){
      if(!NMNS.announcementPage){
        ...
        NMNS.emit('get announcement', {page:1, combined:true});
      }
    });
          ...
          if(e.keyCode === 13 || e.which === 13){
            $(this).prev().trigger('click');
					...
					if($(this).next().val().replace(/-/gi, '').length === 11 || $(this).next().val().replace(/-/gi, '').length === 10){
						...
						NMNS.emit("get noshow", {contact:$(this).next().val().replace(/-/gi, ''), mine:false});
					}else{
      ...
      if(document.scrollingElement.scrollTop > document.getElementById('mainSearchArea').offsetHeight){
		...
		if($("#announcementArea").is(":visible")){
			...
			if(NMNS.expectMoreAnnouncement && distance < Math.max(100, $("#announcementArea").innerHeight() * 0.2)){
				NMNS.emit('get announcement', {page:++NMNS.announcementPage, combined:true})
			}
		}
	}, 100));
})(jQuery);