...
var GuidedTour = function(steps, options) {
    ...
    $(document)
        .on('click', '[data-toggle=popover]', function() {
            toursteps[1 * $(this).data('target')].popover('show');
            return false;
        })
            ...
            $(steps).each(function(i, step) {
                if (step.target) {
                    ...
                    step.content = function() {
                        ...
                        if (i + 1 < steps.length) {
                            out += '<button type="button" class="btn btn-primary btn-sm btn-flat float-right" autofocus data-dismiss="popover" data-toggle="popover" data-target="' + (i + 1) + '">다음</button>';
                        }
                        if (i > 0) {
                            out += '<button type="button" class="btn btn-link btn-sm btn-flat float-right" data-dismiss="popover" data-toggle="popover" data-target="' + (i - 1) + '">이전</button>';
                        }
