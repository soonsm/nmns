      ...
      if (status) {
        ...
        if (isItMyReservation) {
          ...
          if (newReservation.status === process.nmns.RESERVATION_STATUS.NOSHOW) {
            await db.addToNoShowList(email, newReservation.contact, newReservation.noShowCase, newReservation.start.substring(0, 6), newReservation.id);
          }
        }
        else {
          status = false;
