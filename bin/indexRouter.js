...
module.exports = function(passport) {
  ...
  router.get("/home", async function(req, res) {
    ...
    if (req.user) {
      ...
      if (user.authStatus === process.nmns.AUTH_STATUS.EMAIL_VERIFICATED) {
        ...
        if (
          req.user.authStatus !==
            process.nmns.AUTH_STATUS.BEFORE_EMAIL_VERIFICATION ||
          num > 0.7
        ) {
          tips.splice(7, 1);
        ...
        if (user.logoFileName) {
          let baseUrl =
            "https://s3.ap-northeast-2.amazonaws.com/file.washow.co.kr/";
  ...
  router.get("/index", async function(req, res) {
    if (req.user) {
      ...
      render(res, indexView, {
        ...
        kakaotalk:
          req.query.kakaotalk && req.query.kakaotalk !== ""
            ? req.query.kakaotalk
            : undefined
      });
    }
  ...
  router.get("/emailVerification/email=:email&token=:token", async function(
    req,
    res
  ) {
    ...
    if (
      ...
      await db.updateWebUser(email, {
        authStatus: process.nmns.AUTH_STATUS.EMAIL_VERIFICATED
      });
  ...
  router.get(
    "/addNotice/title=:title&&contents=:contents",
    async (req, res) => {
  ...
  router.get(
    "/web_cancel/key=:reservationKey&&email=:email",
    async (req, res) => {
      ...
      try {
        ...
        if (reservation.status === process.nmns.RESERVATION_STATUS.RESERVED) {
          ...
          returnMsg = "예약취소완료";
          contents =
            "노쇼하지 않고 예약취소해주셔서 감사합니다. 다음에 다시 찾아주세요.";

          //Push
          let msg = `${moment(reservation.start, "YYYYMMDDHHmm").format(
            "M월 D일 h시 mm분"
          )}`;
          if (reservation.start.endsWith("00")) {
            msg = `${moment(reservation.start, "YYYYMMDDHHmm").format(
              "M월 D일 h시"
            )}`;
          }
          msg += " 예약이 취소되었습니다.";
  ...
  router.post("/signup", upload, async function(req, res) {
    ...
    try {
      ...
      if (data.useYn === "Y") {
        if (
          !data.callbackPhone ||
          !util.phoneNumberValidation(data.callbackPhone)
        ) {
    ...
    try {
      ...
      for (let item of list) {
        ...
        if (duration.as("seconds") < 60) {
          ...
          item.before = duration.days() / 30 + "달 전";
        }
      }
    } catch (e) {
      status = false;
      message =
        "최근 노쇼 내역 가져오기를 실패했습니다. 잠시 후 이용 바랍니다.";
    }
